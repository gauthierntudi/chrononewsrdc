...
$catsForYouLinks = array_map(
    static fn (string $cat): string => '<a href="'.htmlspecialchars(category_url($cat)).'">'.htmlspecialchars($cat).'</a>',
    $catsForYou,
);
$catsForYouText = $catsForYouLinks !== [] ? implode(', ', $catsForYouLinks) : '';
?>
...
<div class="jlc-container">
    ...
    <div class="jlc-col-md-8 jl_main_achv jl_hide_line jl_achv_tpl_list">
        <div class="jl_breadcrumbs">
            ...
            <span class="jl_item_bread">
                <a href="/juste-pour-vous">Juste pour vous</a>
            </span>
        </div>
        <div class="jl_ache_head">
            <div class="jl_pc_sec_title">
                <h1 class="jl_pc_sec_h">
                    Juste <span style="color:<?= htmlspecialchars(cn_primary_color()) ?>;">pour vous</span>
                </h1>
                <div class="post_subtitle_text">
                    <p>
                        Une sélection d'articles tirée de rubriques choisies pour votre visite
                        <?php if ($catsForYouText !== ''): ?>
                            : <?= $catsForYouText ?>.
                        <?php else: ?>.
                        <?php endif; ?>
                    </p>
            ...
            <span class="jl_auth_numw">
                <span class="jl_auth_num h1" style="color:<?= htmlspecialchars(cn_primary_color()) ?>;">
                    <?= (int) $total_articles ?>
                </span>
        ...
        <div class="jl_clear_at block-section jl-main-block jl_wrapper_cat">
            <div class="jl_clear_at">
                ...
                <div class="jl_fli_wrap jl-roww jl_contain jl-col-row">
                    <?php if (empty($articles)): ?>
                        <div class="jl_no_articles">
                            <p>Aucun article disponible pour cette sélection pour le moment.</p>
                        </div>
                    <?php else: ?>
                            ...
                            <div class="jl_li_in">
                                <div class="jl_img_holder">
                                    <div class="jl_imgw jl_radus_e">
                                        <div class="jl_imgin">
                                            <?php if ($article_image): ?>
                                                <img
                                                    width="680"
                                                    height="453"
                                                    src="<?= htmlspecialchars(cn_media_url($article_image) ?? '') ?>"
                                        ...
                                        <a class="jl_imgl" href="<?= htmlspecialchars($article_url) ?>"></a>
                                ...
                                <div class="jl_fe_text">
                                    <span class="jl_f_cat jl_lb3">
                                        <a
                                            class="jl_cat_lbl"
                                            href="<?= htmlspecialchars($cat_url) ?>"
                                            style="background-color: <?= htmlspecialchars($cat_color) ?>;"
                                        >
                                            <span><?= htmlspecialchars($article['categorie']) ?></span>
                                        </a>
                                    </span>
                                    <h2 class="h2 jl_fe_title">
                                        <a href="<?= htmlspecialchars($article_url) ?>">
                                            <?= htmlspecialchars(titre_limit($t, 100)) ?>
                                        </a>
                                    </h2>
                                    <p class="jl_fe_des">
                                        <?= htmlspecialchars(excerpt($article['contenu'], 160)) ?>
                                    </p>
                                    <span class="jl_post_meta">
                                        ...
                                        <?php if (vues_int($article['vues']) > 0): ?>
                                            <span class="post-views">
                                                <i class="jli-eye"></i>
                                            <?= htmlspecialchars(vues_format($article['vues'])) ?>
                                            </span>
                                        <?php endif; ?>
                                    </span>
                                </div>
                ...
                <div class="jl_pagination">
                    <?php if ($page > 1): ?>
                        <a href="<?= htmlspecialchars(cn_just_for_you_page_url($page - 1)) ?>" class="jl_page_prev">
                    ...
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <?php if ($i == $page): ?>
                            <span class="jl_page_current">
                                <?= (int) $i ?>
                            </span>
                        <?php else: ?>
                            <a href="<?= htmlspecialchars(cn_just_for_you_page_url($i)) ?>" class="jl_page_link">
                                <?= (int) $i ?>
                            </a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    ...
                    <?php if ($page < $total_pages): ?>
                        <a href="<?= htmlspecialchars(cn_just_for_you_page_url($page + 1)) ?>" class="jl_page_next">
            ...
            <div class="jlcus_sec_title jl_sec_style13 jl_secf_title  ">
                <div class="jlcus_sect_inner">
                    <h2 class="jl-heading-text">
                        <span class="jl_ttw">Ne manquez pas</span>
                    </h2>
                </div>
            </div>
        </div>
